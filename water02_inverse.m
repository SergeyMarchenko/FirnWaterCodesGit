...
A = 5;      % number of iterations of the whole optimization routine
N = 3;      % max number of layers included in the forward model above the optimized layer (the less the mode is the reduction of the domain length at the start)
n = N-2;
...
p.f  = 0;   % to plot or not to plot, that is the question
load('W_data.mat');

out.Wm = cell( 1, 11 );
for is = 1:11
% is  = 2;        % string number: 1 - single string from 2015, 2 - averaged data from 2014, 3...11 - strings 1...9 from 2014
...
K = T.or_kR_f{1*(is == 1) + 2*(is ~= 1)}(:,2);
R = T.or_kR_f{1*(is == 1) + 2*(is ~= 1)}(:,3);      %R = T.R;
...
if  p.f; p = plot_background(p, t, z, TIC, F0); end
%%
% tic
for a = 1:10;    if a > 1;  Wm(:,a) = Wm(:,a-1);  end
...
%     i = i+1;
    ...
    else;      TIC_i = [TIC_i(:,to(i-n-1,2))  TIC(:,to(i-n,1)  : to(i,2))];     end

    if  i<N;     t_i =                          t(  1          : to(i,2)) ;
    else;        t_i =                          t(  to(i-n,1)-1: to(i,2)) ;     end

    if  i<N;   to_s  = to(i,1);
    else;      to_s  = to(i,1) - (to(i-n,1)-2);                                 end
    ...
    if p.f;  [p] = plot_update(p, t, z, TIC, R, K, F0, to, Wm, a, i, to_f, t_i, TIC_i, d, thr);
          pause;
    end
