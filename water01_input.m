...
% load temperature data and cut data before July 30 and above the depth of 1 m
load('C:\DATA\temperature\LF_Temp.mat')
    [~, tind15] = min( abs( LF{4}.T.t - datenum([ 2015 07 30 0 0 0 ]) ) );  % 2015 09 09 0 0 0
    [~, tind14] = min( abs( LF{3}.T.t - datenum([ 2014 07 30 0 0 0 ]) ) );  % 2014 08 24 0 0 0
    [~, zind15] = min( abs( LF{4}.T.z_a - 1 ) );
    [~, zind14] = min( abs( LF{3}.T.z_a - 1 ) );
    T.t{   1} = LF{4}.T.t( tind15:end )';    T.z{   1} = LF{4}.T.z_a    ( zind15:end, : );    T.T{   1} = LF{4}.T.T_a    ( zind15:end, tind15:end );
    T.t{   2} = LF{3}.T.t( tind14:end )';    T.z{   2} = LF{3}.T.z_a    ( zind14:end, : );    T.T{   2} = LF{3}.T.T_a    ( zind14:end, tind14:end );
    for is = 1:9
    [~, zind]   = min( abs( LF{3}.T.z_i{is} - 1 ) );
    T.t{is+2} = T.t{2};                      T.z{is+2} = LF{3}.T.z_i{is}( zind  :end, : );    T.T{is+2} = LF{3}.T.T_i{is}( zind  :end, tind14:end );
    end; clear is tind* zind* LF
...
%%
save('HowMuchWater_input.mat')   % data necessaty for quantification of the firn water content
